...
from thrive.tools import float_compare, single_email_re, profiler, lower_logging, SQL
from thrive.tools.misc import find_in_path, mute_logger
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 17.0: thrive.tests.common.Form is deprecated, use thrive.tests.Form",
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(thrive.modules.registry._logger.name)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot'):
    ...
    screenshots_dir = pathlib.Path(thrive.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class HttpCase(TransactionCase):
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "thrive.startTour('%s', %s)" % (tour_name, json.dumps(options)))
        ready = kwargs.pop('ready', "thrive.isTourReady('%s')" % tour_name)
