...
Classic Thrive modules are composed of a top-level :samp:`thrive.define({name},{dependencies},{body_function})` call.
...
from thrive.tools.misc import OrderedSet
...
def transpile_javascript(url, content):
    ...
    steps = [
        ...
        partial(wrap_with_thrive_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
...
def url_to_module_path(url):
    """
    Thrive modules each have a name. (thrive.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_thrive_define(module_path, dependencies, content):
    """
    ...
    """
    return f"""thrive.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def get_aliased_thrive_define_content(module_path, content):
    ...
    > thrive.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nthrive.define(`%s`, ['%s'], function (require) {
                        return require('%s');
