...
class TestWebsiteAssets(thrive.tests.HttpCase):
...
class TestWebAssets(thrive.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with thrive.tools.mute_logger('thrive.addons.web.controllers.binary'):
            self.assertEqual(
