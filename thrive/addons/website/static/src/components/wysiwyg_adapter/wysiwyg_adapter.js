...
import { isMediaElement } from '@web_editor/js/editor/thrive-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@thrive/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        // the dirty observer (not to be confused with thriveEditor
    ...
    async destroy() {
        ...
        const mod = await thrive.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await thrive.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.thriveEditor.filterMutationRecords(records);
        ...
        this.thriveEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.thriveEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by ThriveEditor when text is entered.
    ...
    async _createSnippetsMenuInstance(options = {}) {
        const snippetsEditor = await thrive.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
