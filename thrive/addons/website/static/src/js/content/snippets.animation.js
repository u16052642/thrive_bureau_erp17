...
￼    */
    _setImgSrc(svg, resolve) {
        ...
        preloadedImg.onload = () => {
            ...
            this.options.wysiwyg && this.options.wysiwyg.thriveEditor.observerUnactive("setImgHoverEffectSrc");
            this.el.src = preloadedImg.src;
            this.options.wysiwyg && this.options.wysiwyg.thriveEditor.observerActive("setImgHoverEffectSrc");
...
registry.TextHighlight = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        this._adaptOnResize = throttleForAnimation(() => {
            this.options.wysiwyg?.thriveEditor.observerUnactive("textHighlightResize");
            for (const textEl of this.el.querySelectorAll(".o_text_highlight")) {
                // Remove old effect, normalize content, redraw SVGs...
                switchTextHighlight(textEl, getCurrentTextHighlight(textEl));
            }
            this.options.wysiwyg?.thriveEditor.observerActive("textHighlightResize");
        });
        if (!this.editableMode) {
