...
import * as ThriveEditorLib from "@web_editor/js/editor/thrive-editor/src/utils/utils";
import { Component, onMounted, useRef, useState } from "@thrive/owl";
...
const wSnippetMenu = weSnippetEditor.SnippetsMenu.extend({
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Thrive's D&D
        // There is currently no way in Thrive D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.thriveEditor.document.getSelection();
    },
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.thriveEditor.computeFontSizeSelectorValues();
    },
    /**
    ...
    _handleTextOptions(targetEl, optionClassList, applyTextOption = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = ThriveEditorLib.preserveCursor(this.$body[0].ownerDocument);
