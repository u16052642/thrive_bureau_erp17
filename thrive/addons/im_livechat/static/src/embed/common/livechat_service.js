...
export const THRIVE_VERSION_KEY = `${location.origin.replace(
    /:\/{0,2}/g,
    "_"
)}_im_livechat.thrive_version`;
...
export class LivechatService {
    ...
    async initialize() {
        ...
        if (!this.options.isTestChatbot) {
            ...
            const prevThriveVersion = browser.localStorage.getItem(THRIVE_VERSION_KEY);
            const currThriveVersion = init?.thrive_version;
            const visitorUid = this.visitorUid || false;
            const userId = session.user_id || false;
            if (
                prevThriveVersion !== currThriveVersion ||
                (this.sessionCookie && visitorUid !== userId)
            ) {
                this.leaveSession({ notifyServer: false });
            }
            browser.localStorage.setItem(THRIVE_VERSION_KEY, currThriveVersion);
        }
