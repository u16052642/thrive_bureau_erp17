...
from thrive.tools import ustr, file_open
...
class WebManifest(http.Controller):
    ...
    def webmanifest(self):
        ...
        """
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'Thrive')
        manifest = {
            'name': web_app_name,
            'scope': '/web',
            'start_url': '/web',
        ...
        manifest['icons'] = [{
            'src': '/web/static/img/thrive-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def _icon_path(self):
        return 'web/static/img/thrive-icon-192x192.png'
