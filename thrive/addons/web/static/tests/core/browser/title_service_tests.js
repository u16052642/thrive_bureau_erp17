...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Thrive" });
    assert.strictEqual(env.services.title.current, "Thrive");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Thrive", chat: null });
    assert.strictEqual(env.services.title.current, "Thrive");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Thrive - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Thrive", action: "Import" });
    assert.strictEqual(env.services.title.current, "Thrive - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Thrive", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Thrive - Import"); // parts is a copy!
});
