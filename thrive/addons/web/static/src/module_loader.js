...
(function () {
        ...
        define(name, deps, factory) {
            ...
            if (!this.factories.has(name)) {
                this.factories.set(name, {
                    deps,
                    fn: factory,
                    ignoreMissingDeps: globalThis.__thriveIgnoreMissingDependencies,
                });
                this.addJob(name);
    ...
    const thrive = globalThis.thrive;
    if (thrive.debug && !new URLSearchParams(location.search).has("debug")) {
        // remove debug mode if not explicitely set in url
        thrive.debug = "";
    }

    const loader = new ModuleLoader();
    thrive.define = loader.define.bind(loader);

    thrive.loader = loader;
})();
