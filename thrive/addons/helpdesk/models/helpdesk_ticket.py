...
class HelpdeskTicket(models.Model):
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals and self.env['helpdesk.stage'].browse(vals['stage_id']).fold:
            thrivebot_partner_id = self.env['ir.model.data']._xmlid_to_res_id('base.partner_root')
            for ticket in self:
                ...
                if exceeded_hours:
                    ...
                    ticket.message_post(body=message, subtype_xmlid="mail.mt_note", author_id=thrivebot_partner_id)
        return res
