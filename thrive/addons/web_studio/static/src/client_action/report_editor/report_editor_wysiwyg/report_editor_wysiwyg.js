...
import { setSelection, startPos, endPos } from "@web_editor/js/editor/thrive-editor/src/utils/utils";
...
export class ReportEditorWysiwyg extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            this.Wysiwyg = (
                await thrive.loader.modules.get("@web_editor/js/wysiwyg/wysiwyg")
            ).Wysiwyg;
        });
        ...
        this.setWysiwygInstance = async (wysiwyg) => {
            ...
            const thriveEditor = this.wysiwyg.thriveEditor;

            const undoRedoState = this.undoRedoState;
            undoRedoState.canUndo = false;
            undoRedoState.canRedo = false;

            thriveEditor.addEventListener("historyStep", () => {
                undoRedoState.canUndo = thriveEditor.historyCanUndo();
                undoRedoState.canRedo = thriveEditor.historyCanRedo();
            ...
            this.observer = new MutationObserver((records) =>
                this.domChangesDirtyMutations(thriveEditor, records)
            );
            thriveEditor.addEventListener("observerUnactive", () => {
                if (this.observer) {
                    this.domChangesDirtyMutations(thriveEditor, this.observer.takeRecords());
                    this.observer.disconnect();
                }
            });

            thriveEditor.addEventListener("observerActive", observe);

            thriveEditor.observerUnactive();
            if (thrive.debug) {
                ["t-esc", "t-out", "t-field"].forEach((tAtt) => {
                    thriveEditor.document.querySelectorAll(`*[${tAtt}]`).forEach((e) => {
            ...
            thriveEditor.observerActive();
        };

        this.undoRedoState = reactive({
            canUndo: false,
            canRedo: false,
            undo: () => this.wysiwyg?.thriveEditor.historyUndo(),
            redo: () => this.wysiwyg?.thriveEditor.historyRedo(),
        });
    }
    ...
    async discard() {
        this.wysiwyg.thriveEditor.document.getSelection().removeAllRanges();
        await this.wysiwyg.cancel(false);
        await this.reportEditorModel.discardReport();
    }

    domChangesDirtyMutations(thriveEditor, records) {
        records = thriveEditor.filterMutationRecords(records);
    ...
    getFieldPopoverParams() {
        const thriveEditor = this.wysiwyg.thriveEditor;
        const doc = thriveEditor.document;
    ...
    async insertTableX2Many() {
        ...
        await this.fieldPopover.open(popoverAnchor, {
            ...
            validate: (
                ...
                this.wysiwyg.focus();
                const doc = this.wysiwyg.thriveEditor.document;
                ...
                this.wysiwyg.thriveEditor.execCommand("insert", table);
    ...
    async insertField() {
        ...
        await this.fieldPopover.open(popoverAnchor, {
            ...
            validate: (qwebVar, fieldNameChain, defaultValue = "", is_image) => {
                ...
                this.wysiwyg.thriveEditor.execCommand("insert", span);
            },
        });
    }
