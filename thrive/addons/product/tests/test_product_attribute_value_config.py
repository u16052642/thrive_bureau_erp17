...
from thrive.addons.base.tests.common import DISABLED_MAIL_CONTEXT
...
class TestProductAttributeValueConfig(TestProductAttributeValueCommon):
    ...
    def test_constraints(self):
        ...
        with mute_logger('thrive.sql_db'), self.assertRaises(IntegrityError, msg="can't have two values with the same name for the same attribute"):
            self.env['product.attribute.value'].create({
                'name': '32 GB',
                'attribute_id': self.ram_attribute.id,
            })
