...
import { EventBus, markup, whenReady, reactive } from "@thrive/owl";
...
export const tourService = {
    ...
    start: async (_env, { orm, effect, ui, overlay }) => {
        ...
        thrive.isTourReady = (tourName) => tours[tourName].wait_for.then(() => true);
        ...
        if (isTourEnabled && !ui.isSmall) {
            ...
            for (const tour of sortedTours) {
                thrive.isTourReady(tour.name).then(() => {
