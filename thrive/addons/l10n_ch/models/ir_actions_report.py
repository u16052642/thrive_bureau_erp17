            ...
            if qr_inv_ids:
                ...
                if header:
                    ...
                    for invoice_id, stream in qr_res.items():
                        qr_pdf = ThrivePdfFileReader(stream['stream'], strict=False)
                        header_pdf = ThrivePdfFileReader(header_res[invoice_id]['stream'], strict=False)

                        page = header_pdf.getPage(0)
                        page.mergePage(qr_pdf.getPage(0))

                        output_pdf = ThrivePdfFileWriter()
