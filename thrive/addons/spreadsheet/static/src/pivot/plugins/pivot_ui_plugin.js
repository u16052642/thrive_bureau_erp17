...
export class PivotUIPlugin extends spreadsheet.UIPlugin {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_THRIVE_PIVOT_DOMAIN" || cmd.type === "INSERT_PIVOT"
                );
     ...
     *
     * =THRIVE.PIVOT(1,"expected_revenue","stage_id",2,"city","Brussels")
     * =THRIVE.PIVOT.HEADER(1,"stage_id",2,"city","Brussels")
     * =THRIVE.PIVOT.HEADER(1,"stage_id",2,"city","Brussels","measure","expected_revenue")
     *
     * the result is the same: ["stage_id", 2, "city", "Brussels"]
     *
     * If the cell is the result of THRIVE.PIVOT.TABLE, the result is the domain of the cell
    ...
    getPivotDomainArgsFromPosition(position) {
        ...
        if (functionName === "THRIVE.PIVOT.TABLE") {
        ...
        const domain = args.slice(functionName === "THRIVE.PIVOT" ? 2 : 1);
        if (domain.at(-2) === "measure") {
            return domain.slice(0, -2);
        }
        return domain;
