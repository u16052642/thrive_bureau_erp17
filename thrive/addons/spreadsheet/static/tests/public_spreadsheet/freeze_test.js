...
import { freezeThriveData } from "../../src/helpers/model";
...
QUnit.module("freezing spreadsheet", {}, function () {
    QUnit.test("thrive pivot functions are replaced with their value", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.strictEqual(getCell(model, "A3").content, '=THRIVE.PIVOT.HEADER(1,"bar","false")');
        assert.strictEqual(
        ...
        const data = await freezeThriveData(model);
    ...
    QUnit.test("invalid expression with pivot function", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", "=THRIVE.PIVOT(1)+"); // invalid expression
    ...
    QUnit.test("thrive pivot functions detection is not case sensitive", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", '=thrive.pivot(1,"probability")');
        setCellContent(model, "A2", '=THRIVE.pivot(1,"probability")');
        const data = await freezeThriveData(model);
    ...
    QUnit.test("computed format is exported", async function (assert) {
        ...
        setCellContent(model, "A1", '=THRIVE.PIVOT(1,"pognon")');
    ...
    QUnit.test("thrive charts are replaced with an image", async function (assert) {
