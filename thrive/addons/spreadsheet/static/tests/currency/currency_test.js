...
QUnit.test("rate formula at a given date(time)", async (assert) => {
    ...
    setCellContent(model, "A1", `=THRIVE.CURRENCY.RATE("EUR","USD", "12-31-2020")`);
    setCellContent(model, "A2", `=THRIVE.CURRENCY.RATE("EUR","USD", "11-30-2020 00:00:00")`);
    await waitForDataSourcesLoaded(model);
    assert.verifySteps(["rate fetched"]);
});
...
QUnit.test("invalid date", async (assert) => {
    ...
    setCellContent(model, "A1", `=THRIVE.CURRENCY.RATE("EUR","USD", "hello")`);
    await waitForDataSourcesLoaded(model);
    assert.strictEqual(getCellValue(model, "A1"), "#ERROR");
    assert.strictEqual(
        getEvaluatedCell(model, "A1").error.message,
        "The function THRIVE.CURRENCY.RATE expects a number value, but 'hello' is a string, and cannot be coerced to a number."
    );
});
