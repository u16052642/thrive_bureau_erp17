...
QUnit.test("Cannot open see records on the main THRIVE.PIVOT.TABLE cell", async function (assert) {
    const { env, model } = await createSpreadsheetWithPivot();
...
QUnit.test(
    "Cannot open see records on the empty THRIVE.PIVOT.TABLE cell below the main cell",
...
QUnit.test("Can see records on THRIVE.PIVOT.TABLE cells", async function (assert) {
    const actions = [];
    const fakeActionService = {
        start: (env) => ({
            doAction: (actionRequest, options = {}) => {
    ...
    async function checkCells(cells) {
        ...
        // sheet where the pivot is made of a single THRIVE.PIVOT.TABLE formula.
        for (const [xc, formula] of Object.entries(cells)) {
            ...
            // action on the second sheet, on THRIVE.PIVOT.TABLE
    ...
    const header_cells = {
        ...
        B1: '=THRIVE.PIVOT.HEADER(1,"foo",1)',
        // B2 is a measure header
        B2: '=THRIVE.PIVOT.HEADER(1,"foo",1,"measure","probability")',
        // A3 is a row header
        A3: '=THRIVE.PIVOT.HEADER(1,"bar","false")',
        // A5 is a total header
        A5: "=THRIVE.PIVOT.HEADER(1)",
    };
    const data_cells = {
        // B3 is an empty value
        B3: '=THRIVE.PIVOT(1,"probability","bar","false","foo",1)',
        // B4 is an non-empty value
        B4: '=THRIVE.PIVOT(1,"probability","bar","true","foo",1)',
        // B5 is a column group total value
        B5: '=THRIVE.PIVOT(1,"probability","foo",1)',
        // F3 is a row group total value
        F3: '=THRIVE.PIVOT(1,"probability","bar","false")',
        // F5 is the total
        F5: '=THRIVE.PIVOT(1,"probability")',
    };
    ...
    setCellContent(model, "A3", `=THRIVE.PIVOT.TABLE("1",,,FALSE)`, "42");
