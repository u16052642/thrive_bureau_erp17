...
QUnit.module("documents_spreadsheet > Topbar Menu Items", {}, function () {
    ...
    QUnit.test("Can Insert thrive formulas from Insert > Functions > Thrive", async function (assert) {
        ...
        await click(document.querySelector(".o-menu-root[title='Thrive']"));
        await click(document.querySelector(".o-menu-item[title='THRIVE.CURRENCY.RATE']"));

        await triggerEvent(document.activeElement, null, "keydown", { key: "Enter" });

        assert.equal(getCellContent(model, "A1"), "=THRIVE.CURRENCY.RATE()");
    });
});
