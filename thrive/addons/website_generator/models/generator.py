...
DEFAULT_WSS_ENDPOINT = 'https://iap-scraper.thrivebureau.com/'
...
class WebsiteGeneratorRequest(models.Model):
    ...
    def create(self, vals_list):
        ...
        for req in wg_requests:
            ...
            # created via the thrivebureau.com start trial.
            if not req.uuid:
    ...
    def _generate_site(self, tar):
        thrive_blocks = self._load_input(tar)
        website = self._get_website(thrive_blocks, tar)
        # Generate the images attachments (Modifies thrive_blocks in place)
        self._save_images_as_attachments(thrive_blocks, tar)
        self._generate_pages(website, thrive_blocks)
        return website, thrive_blocks
    ...
    def _generate_pages(self, website, thrive_blocks):
        # Create pages
        for page_url, page_data in thrive_blocks.get('pages', {}).items():
        ...
        homepage._construct_homepage(thrive_blocks['homepage'])

    def _get_website(self, thrive_blocks, tar):
        website = self.env['website'].get_current_website()
        website_info = thrive_blocks.get('website')
        if not website_info:
            raise ValueError("Website info not found in the input")
        homepage_url = thrive_blocks.get('homepage', {}).get('url')
        if not homepage_url:
    ...
    def _save_images_as_attachments(self, thrive_blocks, tar):
        all_images = thrive_blocks['website'].get('all_images', {})
        ...
        # Home page
        for image in thrive_blocks['homepage'].get('images_to_crop', []):
        ...
        # Other pages
        for page_dict in thrive_blocks.get('pages', {}).values():
        ...
        if attachments_url_src:
            # Modifies thrive_blocks in place
            self._update_html_urls(thrive_blocks, attachments_url_src, cropped_attachments_url_src)
    ...
    def _update_html_urls(self, thrive_blocks, attachments_url_src, cropped_attachments_url_src):
        ...
        homepage = thrive_blocks['homepage']
        ...
        for page_name, page_dict in thrive_blocks.get('pages', {}).items():
            thrive_blocks['pages'][page_name]['body_html'] = update_page_html(page_dict)
