...
export class HtmlField extends Component {
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("thrive-editor-qweb");
        ...
        this.wysiwyg.thriveEditor.addEventListener("historyStep", () =>
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.thriveEditor.sanitize(t);
        }
    }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await thrive.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await thrive.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await thrive.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await thrive.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.thriveEditor.sanitize(this.wysiwyg.thriveEditor.editable);
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(thrive.debug && options.codeview),
            placeholder: attrs.placeholder,
