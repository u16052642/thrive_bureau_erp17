...
import { closestElement, isUnremovable } from "@web_editor/js/editor/thrive-editor/src/utils/utils";
...
var SnippetEditor = Widget.extend({
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.thriveEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.thriveEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        ...
        this.options.wysiwyg.thriveEditor.observerUnactive('dragAndDropMoveSnippet');
...
var SnippetsMenu = Widget.extend({
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.thriveEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.thriveEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                this.options.wysiwyg.thriveEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.thriveEditor.observerActive('dragAndDropCreateSnippet');
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.thriveEditor.bindExecCommand(customizeTableBlock);
