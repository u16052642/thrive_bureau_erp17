...
import * as ThriveEditorLib from "@web_editor/js/editor/thrive-editor/src/ThriveEditor";
...
import { isSelectionInSelectors, peek } from '@web_editor/js/editor/thrive-editor/src/utils/utils';
...
const isProtected = ThriveEditorLib.isProtected;
...
const closestBlock = ThriveEditorLib.closestBlock;
const getRangePosition = ThriveEditorLib.getRangePosition;
...
export class Wysiwyg extends Component {
    ...
    async startEdition() {
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.thriveEditor.document.getSelection().anchorNode)) {
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.thriveEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        this.thriveEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.thriveEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.thriveEditor.isMobile) {
                this.thriveEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.thriveEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length && this.thriveEditor) {
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.thriveEditor) : [];
    ...
    async cleanForSave(editable = this.thriveEditor.editable) {
        if (this.thriveEditor) {
            this.thriveEditor.cleanForSave(editable);
        ...
        if (this.thriveFieldObservers) {
            ...
            $thriveFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    ...
                    this.thriveEditor.withoutRollback(() => {
    ...
    openLinkToolsFromSelection() {
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.thriveEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    ...
                    const addHintClasses = () => {
                        this.thriveEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.thriveEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.thriveEditor.editable,
                        link,
            ...
            const historyStepIndex = this.thriveEditor.historySize() - 1;
            this.thriveEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.thriveEditor.historyUnpauseSteps();
                    this.thriveEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt') {
        const restore = preserveCursor(this.thriveEditor.document);
        const params = {
            insert: content => {
                this.thriveEditor.historyPauseSteps();
                const insertedNodes = this.thriveEditor.execCommand('insert', content);
                this.thriveEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.thriveEditor.editable.parentElement;
        ...
        if (mode === 'alternatives') {
            params.originalText = this.thriveEditor.document.getSelection().toString() || '';
        }
        this.thriveEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                this.thriveEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
    ...
    showImageFullscreen(url) {
        ...
        this.thriveEditor.document.getSelection()?.collapseToEnd();
        this.thriveEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        ...
        const editable = ThriveEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.thriveEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.thriveEditor.document.getSelection();
        ...
        const cursorAt = getRangePosition(targetEl, this.thriveEditor.document).left;
        let position = 'end';
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.thriveEditor.execCommand('insert', str);
                },
            },
    ...
    async _createSnippetsMenuInstance(options={}) {
        const snippetsEditor = await thrive.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.thriveEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const coloredElements = this.thriveEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
    ...
    _updateEditorUI(e) {
        ...
        const linkNode = getInSelection(this.thriveEditor.document, 'a');
        ...
        this.thriveEditor.updateToolbarPosition();
    ...
    _getBannerCommand(title, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.thriveEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.thriveEditor.document, `
                ...
                `).childNodes[0];
                this.thriveEditor.execCommand('insert', bannerElement);
            ...
            },
            {
                ...
                callback: async () => {
                    ...
                    if (user && user.signature) {
                        this.thriveEditor.execCommand('insert', parseHTML(this.thriveEditor.document, user.signature));
                    }
                },
            },
            {
    ...
    _onSelectionChange() {
        if (this.thriveEditor.autohideToolbar) {
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.thriveEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.thriveEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.thriveEditor.editable.cloneNode(true).outerHTML);
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                recover_document: (params) => {
                    ...
                    if (!this.thriveEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.thriveEditor.historyGetMissingSteps({ fromStepId }),
                ...
                switch (notificationName) {
                    ...
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.thriveEditor.multiselectionRefresh();
                        });
    ...
    _isLastDocumentStale() {
        return !this.thriveEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.thriveEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the thrive bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.thriveEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.thriveEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.thriveEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.thriveEditor.historyResetFromSteps(steps, historyIds);
        this.thriveEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        if (node?.querySelectorAll && this.thriveEditor && !this.thriveEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.thriveEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.thriveEditor.historyGetBranchIds().join(',');
