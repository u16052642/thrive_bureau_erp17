...
import {
    ...
    patchEditorIframe,
} from '@web_editor/js/editor/thrive-editor/test/utils';
...
class PeerTest {
    ...
    async startEditor() {
        ...
        if (this.initialParsedSelection) {
            ...
            this.wysiwyg.thriveEditor._recordHistorySelection();
        } else {
    ...
    async getValue() {
        this.wysiwyg.thriveEditor.observerUnactive('PeerTest.getValue');
        ...
        this.wysiwyg.thriveEditor.observerActive('PeerTest.getValue');
...
export async function createPeers(peers) {
    ...
    for (const peerId of peers) {
        ...
        patch(wysiwyg, {
            ...
            // Hacky hook as we know this method is called after setting the value in the wysiwyg start and before sending the value to thriveEditor.
    ...
    QUnit.module('Collaboration', {}, () => {
        ...
        QUnit.module('Focus', {}, () => {
            QUnit.test('Focused client should not receive step if no data channel is open', async (assert) => {
                ...
                await peers.p1.wysiwyg.thriveEditor.execCommand('insert', 'b');
        ...
        QUnit.module('Stale detection & recovery', {}, () => {
            QUnit.module('detect stale while unfocused', async () => {
                QUnit.test('should do nothing until focus', async (assert) => {
                    ...
                    await peers.p1.wysiwyg.thriveEditor.execCommand('insert', 'c');
        ...
        QUnit.module('Disconnect & reconnect', {}, () => {
            QUnit.test('should sync history when disconnecting and reconnecting to internet', async (assert) => {
                ...
                const setSelection = peer => {
                    ...
                    const pElement = peer.wysiwyg.thriveEditor.editable.querySelector('p')
                ...
                const addP = (peer, content) => {
                    ...
                    peer.wysiwyg.thriveEditor.editable.append(p);
                    peer.wysiwyg.thriveEditor.historyStep();
                }
                ...
                await peers.p2.wysiwyg.thriveEditor.execCommand('insert', 'e');
                addP(peers.p2, 'f');
