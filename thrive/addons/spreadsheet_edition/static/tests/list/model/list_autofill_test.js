...
QUnit.module("spreadsheet > list autofill", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithList();
            setCellContent(model, "A1", '=THRIVE.LIST(1, A1,"probability")');
            selectCell(model, "A1");

            model.dispatch("AUTOFILL_SELECT", { col: 0, row: 1 });
            model.dispatch("AUTOFILL");
            assert.equal(getCellFormula(model, "A2"), '=THRIVE.LIST(1, A2,"probability")');
        }
    );
    ...
    QUnit.test("Autofill list formula with missing listId", async function (assert) {
            ...
            sheets: [
                {
                    ...
                    rowNumber: 2,
                    cells: {
                        A1: { content: '=THRIVE.LIST("1","1","date")' },
                        B1: { content: '=THRIVE.LIST.HEADER("1","date")' },
                    },
                },
            ],
        });
        assert.strictEqual(
            ...
            '=THRIVE.LIST("1","1","date")'
        );
        assert.strictEqual(
            getListAutofillValue(model, "B1", { direction: "bottom", steps: 1 }),
            '=THRIVE.LIST.HEADER("1","date")'
        );
        assert.strictEqual(
