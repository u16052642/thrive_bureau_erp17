...
QUnit.module("spreadsheet > pivot_autofill", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "A1", `=THRIVE.PIVOT(1,"probability","bar",B2,"foo",$C$3)`);
            selectCell(model, "A1");
            ...
            assert.equal(
                ...
                `=THRIVE.PIVOT(1,"probability","bar",B3,"foo",$C$3)`
            );
        }
    );
    ...
    QUnit.test("Can autofill positional col headers horizontally", async (assert) => {
        ...
        setCellContent(model, "B1", `=THRIVE.PIVOT.HEADER(1,"#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "right", steps: 1 }),
            `=THRIVE.PIVOT.HEADER(1,"#product_id",2)`
        );
        selectCell(model, "B1");
    ...
    QUnit.test("Can autofill positional row headers vertically", async (assert) => {
        ...
        setCellContent(model, "A3", `=THRIVE.PIVOT.HEADER(1,"date:month","04/2016","#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=THRIVE.PIVOT.HEADER(1,"date:month","04/2016","#product_id",2)`
        );
        selectCell(model, "A3");
    ...
    QUnit.test("Can autofill positional col horizontally", async (assert) => {
        ...
        setCellContent(model, "B1", `=THRIVE.PIVOT(1,"probability","#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "right", steps: 1 }),
            `=THRIVE.PIVOT(1,"probability","#product_id",2)`
        );
        selectCell(model, "B1");
        model.dispatch("AUTOFILL_SELECT", { col: 2, row: 0 });
        const tooltipContent = model.getters.getAutofillTooltip().props.content;
    ...
    QUnit.test("Can autofill positional row vertically", async (assert) => {
        ...
        setCellContent(
            model,
            "A3",
            `=THRIVE.PIVOT(1,"probability","date:month","04/2016","#product_id",1)`
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "bottom", steps: 1 }),
            `=THRIVE.PIVOT(1,"probability","date:month","04/2016","#product_id",2)`
        );
        selectCell(model, "A3");
        model.dispatch("AUTOFILL_SELECT", { col: 1, row: 3 });
        const tooltipContent = model.getters.getAutofillTooltip().props.content;
    ...
    QUnit.test("Autofill pivot values with date (week) 2020 has 53 weeks", async function (assert) {
        ...
        setCellContent(model, "A1", '=THRIVE.PIVOT.HEADER(1,"date:week","52/2020")');
        assert.strictEqual(
            getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
            '=THRIVE.PIVOT.HEADER(1,"date:week","53/2020")'
        );
    });
    ...
    QUnit.test("Autofill empty pivot date value", async function (assert) {
        for (const interval of ["day", "week", "month", "quarter", "year"]) {
            ...
            setCellContent(model, "A1", `=THRIVE.PIVOT.HEADER(1,"date:${interval}","false")`);
            assert.strictEqual(
                getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
                `=THRIVE.PIVOT.HEADER(1,"date:${interval}","false")`
            );
    ...
    QUnit.test("Autofill pivot formula with missing pivotId", async function (assert) {
            ...
            sheets: [
                {
                    ...
                    rowNumber: 2,
                    cells: {
                        A1: { content: '=THRIVE.PIVOT("1","bar","date","05/2023")' },
                        B1: { content: '=THRIVE.PIVOT.HEADER("1","date","05/2023")' },
                    },
                },
            ],
        });
        assert.strictEqual(
            getPivotAutofillValue(model, "A1", { direction: "bottom", steps: 1 }),
            '=THRIVE.PIVOT("1","bar","date","05/2023")'
        );
        assert.strictEqual(
            getPivotAutofillValue(model, "B1", { direction: "bottom", steps: 1 }),
            '=THRIVE.PIVOT.HEADER("1","date","05/2023")'
        );
