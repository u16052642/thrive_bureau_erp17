...
topbarMenuRegistry.addChild("data_sources_data", ["data"], (env) => {
    ...
    const lists_items = env.model.getters.getListIds().map((listId, index) => {
        return {
            ...
            icon: "o-spreadsheet-Icon.THRIVE_LIST",
            separator: index === env.model.getters.getListIds().length - 1,
        };
    });
    const charts_items = env.model.getters.getThriveChartIds().map((chartId, index) => {
        return {
            id: `item_chart_${chartId}`,
            name: env.model.getters.getThriveChartDisplayName(chartId),
            sequence: sequence++,
            execute: (env) => {
            ...
            separator: index === env.model.getters.getThriveChartIds().length - 1,
        };
    });
    return pivots_items
        .concat(lists_items)
