...
from thrive.addons.account_online_synchronization.tests.common import AccountOnlineSynchronizationCommon
...
class TestSynchStatementCreation(AccountOnlineSynchronizationCommon):
    ...
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineLink._fetch_transactions')
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineLink._get_consent_expiring_date')
    ...
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineLink._fetch_thrive_fin')
    ...
    @patch('thrive.addons.account_online_synchronization.models.account_online.requests')
    def test_fetch_receive_error_message(self, patched_request):
        ...
        generated_url = 'https://www.thrivebureau.com/help?stage=bank_sync&summary=Bank+sync+error+ref%3A+abc123+-+Provider%3A+theonlyone+-+Client+ID%3A+client_id_1&description=ClientID%3A+client_id_1%0AInstitution%3A+Test+Bank%0AError+Reference%3A+abc123%0AError+Message%3A+This+kind+of+things+can+happen%0A'
        return_act_url = {
            'type': 'ir.actions.act_url',
            'url': generated_url
        }
        body_generated_url = generated_url.replace('&', '&amp;') #in post_message, & has been escaped to &amp;
        message_body = f'<p>This kind of things can happen<br>You can contact Thrive support <a href="{body_generated_url}">Here</a></p>'
        ...
        try:
            ...
            with self.env.registry.cursor() as test_cr:
                ...
                try:
                    test_link_account._fetch_thrive_fin('/testthisurl')
