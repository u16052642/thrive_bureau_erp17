...
class TestAccountOnlineAccount(AccountOnlineSynchronizationCommon):
    ...
    def test_retrieve_pending_transactions(self, patched_fetch_thrivefin):
        self.account_online_link.state = 'connected'
        patched_fetch_thrivefin.side_effect = [{
            'transactions': [
    ...
    @patch('thrive.addons.base.models.ir_cron.ir_cron._trigger')
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._retrieve_transactions', return_value={})
    ...
    @patch('thrive.addons.base.models.ir_cron.ir_cron._trigger')
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._retrieve_transactions', return_value={})
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._refresh', return_value=True)
    ...
    def test_currently_processing_fetching_transactions(self, patched_refresh, patched_transactions, patched_trigger):
        self.account_online_account.fetching_status = 'processing'  # simulate the fact that we are currently creating entries in thrive
