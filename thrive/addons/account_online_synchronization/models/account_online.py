...
from thrive.exceptions import UserError, CacheMiss, MissingError, ValidationError, RedirectWarning
...
from thrive.tools.misc import format_amount, format_date, get_lang

_logger = logging.getLogger(__name__)
pattern = re.compile("^[a-z0-9-_]+$")
runbot_pattern = re.compile(r"^https:\/\/[a-z0-9-_]+\.[a-z0-9-_]+\.thrive\.com$")

class ThriveFinRedirectException(UserError):
...
class AccountOnlineAccount(models.Model):
    ...
    fetching_status = fields.Selection(
        selection=[
            ...
            ('processing', 'Processing'),  # When currently importing in thrive
            ('done', 'Done'),  # When every transaction have been imported in thrive
        ]
    )
    ...
    def _refresh(self):
        """
            ...
            - Provider is in the process of importing the transactions so we should wait until he has
                finished before fetching them in Thrive
    ...
    def _get_filtered_transactions(self, new_transactions):
        ...
        # Remove transactions already imported in Thrive
...
class AccountOnlineLink(models.Model):
    ...
    has_unlinked_accounts = fields.Boolean(default=True, help="True if that connection still has accounts that are not linked to an Thrive journal")
    ...
    def _handle_thrivefin_redirect_exception(self, mode='link'):
        if mode == 'link':
    ...
    def _fetch_thrive_fin(self, url, data=None, ignore_status=False):
        ...
        if request:
            ...
            # of client-initiated actions. It won't be stored on thrivefin side.
            data['utils']['psu_info'] = {
    ...
    def _handle_response(self, resp_json, url, data, ignore_status=False):
        ...
        if not resp_json.get('error'):
            ...
            if error.get('code') == 101:  # access token expired, not an error
                ...
                raise ThriveFinRedirectException(mode=error.get('data', {}).get('mode', 'link'))
    ...
    def _log_information(self, state, subject=None, message=None, reset_tx=False):
        ...
        try:
            if reset_tx:
                ...
                thrive_help_description = f'''ClientID: {self.client_id}\nInstitution: {self.name}\nError Reference: {error_reference}\nError Message: {message}\n'''
                thrive_help_summary = f'Bank sync error ref: {error_reference} - Provider: {provider} - Client ID: {self.client_id}'
                url_params = urllib.parse.urlencode({'stage': 'bank_sync', 'summary': thrive_help_summary, 'description': thrive_help_description[:1500]})
                url = f'https://www.thrivebureau.com/help?{url_params}'
            ...
            if subject and message:
                ...
                if reset_tx:
                    message_post = Markup('%s<br>%s <a href="%s" >%s</a>') % (message_post, _("You can contact Thrive support"), url, _("Here"))
            ...
            if reset_tx:
                ...
                # and then raise a redirectWarning error so that customer can easily open an issue with Thrive.
                self.env.cr.commit()
                action_id = {
    ...
    def _pre_check_fetch_transactions(self):
        ...
        # This is needed because otherwise the creation of bnk_stmt_lines in Thrive could be interrupted if the process takes too much time
        ...
        try:
            ...
            for online_account in acc:
                ...
                try:
                    ...
                    raise
                except ThriveFinRedirectException as redirect_exception:
                    self._notify_connection_update(
                        journal=journal,
                        connection_state_details={
                            'status': 'error',
                            'error_type': 'thrivefin_redirect',
                            'action': self._handle_thrivefin_redirect_exception(mode=redirect_exception.mode),
                        },
                    )
                    raise
            ...
            return
        except ThriveFinRedirectException as e:
            return self._handle_thrivefin_redirect_exception(mode=e.mode)
    ...
    def _authorize_access(self, data_access_token):
        ...
        try:
            resp_json = self._fetch_thrive_fin('/proxy/v1/authorize_access', data)
