...
from thrive import models, fields, api, _, SUPERUSER_ID
...
class FrontdeskVisitor(models.Model):
    ...
    def _notify_by_discuss(self, recipients, msg, is_host=False):
        for recipient in recipients:
            thrivebot_id = self.env.ref("base.partner_root").id
            ...
            channel.message_post(body=msg, author_id=thrivebot_id, message_type="comment", subtype_xmlid="mail.mt_comment")
    ...
    def _notify_by_email(self):
        for host in self.host_ids:
            if host.work_email:
                ...
                host.message_notify(
                    email_from=thrivebot.email_formatted,
