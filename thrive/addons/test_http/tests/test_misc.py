...
import thrive
from thrive.http import root, content_disposition
from thrive.tests import tagged
from thrive.tests.common import HOST, new_test_user, get_db_name, BaseCase
from thrive.tools import config, file_path
from thrive.addons.test_http.controllers import CT_JSON

from thrive.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'thrivebureau.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict('thrive.tools.config.options', {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from thrive.addons.test_http import controllers  # pylint: disable=C0415
