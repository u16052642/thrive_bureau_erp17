...
class ResUsersSettings(models.Model):
    ...
    should_call_from_another_device = fields.Boolean(
        ...
        help="""If enabled, placing a call in Thrive will transfer the call to the "External device number". Use this option to place the call in Thrive but handle it from another device - e.g. your desk phone.""",
    )
    external_device_number = fields.Char(
        "External device number",
        help="""If the "Call from another device" option is enabled, calls placed in Thrive will be transfered to this phone number.""",
    )

    should_auto_reject_incoming_calls = fields.Boolean(
        "Reject incoming calls",
        help="If enabled, incoming calls will be automatically declined in Thrive.",
    )
