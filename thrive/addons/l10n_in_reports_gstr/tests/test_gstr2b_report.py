...
class TestReports(TestAccountReportsCommon):
    ...
    def test_gstr2b(self):
        ...
        bill_not_in_thrive = self.env['account.move'].search([('ref', '=', '533515'), ('company_id', '=', self.company_data['company'].id)])
        self.assertEqual(len(bill_not_in_thrive), 1)
        self.assertEqual(bill_not_in_thrive.l10n_in_gstr2b_reconciliation_status, 'gstr2_bills_not_in_thrive')
        self.assertEqual(bill_not_in_thrive.l10n_in_gst_treatment, 'regular')
        sez_bill = self.env['account.move'].search([('ref', '=', 'SEZ/123'), ('company_id', '=', self.company_data['company'].id)])
        self.assertEqual(len(sez_bill), 1)
        self.assertEqual(sez_bill.l10n_in_gstr2b_reconciliation_status, "gstr2_bills_not_in_thrive")
