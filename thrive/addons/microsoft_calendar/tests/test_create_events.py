...
class TestCreateEvents(TestCommon):
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_thrive(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Thrive attendee calendar.
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        ...
        """
        if not self.sync_thrive_recurrences_with_outlook_feature():
            return
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Thrive due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Thrive Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Thrive Calendar synced.
