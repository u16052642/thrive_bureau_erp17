...
from thrive.tests.common import HttpCase, JsonRpcException, new_test_user, tagged
...
class TestTranslationController(HttpCase):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="thrive.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="thrive.exceptions.AccessError"), mute_logger("thrive.http"):
            self._mock_translation_request({"message_id": self.message.id})
