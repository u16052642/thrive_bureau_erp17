...
QUnit.test("rendering with ThriveBot has a request (default)", async (assert) => {
    ...
    await contains(".o-mail-NotificationItem", { text: "ThriveBot has a request" });
});

QUnit.test("rendering without ThriveBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
QUnit.test("rendering without ThriveBot has a request (accepted)", async () => {
...
QUnit.test("respond to notification prompt (denied)", async () => {
    ...
    await contains(".o_notification.border-warning", {
        text: "Thrive will not send notifications on this device.",
    });
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
...
QUnit.test("respond to notification prompt (granted)", async () => {
    ...
    await contains(".o_notification.border-success", {
        text: "Thrive will send notifications on this device!",
    });
});

QUnit.test("no 'ThriveBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
    // simulate Android Thrive App
    patchWithCleanup(browser.navigator, {
        userAgent: "Chrome/0.0.0 Android (ThriveMobile; Linux; Android 13; Thrive TestSuite)",
    });
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
...
QUnit.test("rendering with PWA installation request", async (assert) => {
    ...
    assert.strictEqual(
        ...
        "ThriveBot has a suggestion"
    );
    assert.strictEqual(
        target.querySelector(".o-mail-NotificationItem-text").textContent,
        "Come here often? Install Thrive on your device!"
    );
