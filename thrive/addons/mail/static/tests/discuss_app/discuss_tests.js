...
QUnit.test("Posting message should transform links.", async () => {
    ...
    const { openDiscuss } = await start();
    openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.thrivebureau.com/");
    await click(".o-mail-Composer-send:enabled");
    await contains("a[href='https://www.thrivebureau.com/']");
});
...
QUnit.test(
    "receive new chat message: out of thrive focus (notification, channel)",
    async (assert) => {
        const pyEnv = await startServer();
        ...
        // simulate receiving a new message with thrive out-of-focused
...
QUnit.test("receive new chat message: out of thrive focus (notification, chat)", async (assert) => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({ channel_type: "chat" });
    const { env, openDiscuss } = await start({
        services: {
...
QUnit.test("no out-of-focus notification on receiving self messages in chat", async (assert) => {
    ...
    const channel = pyEnv["discuss.channel"].searchRead([["id", "=", channelId]])[0];
    // simulate receiving a new message of self with thrive out-of-focused
    pyEnv["bus.bus"]._sendone(channel, "discuss.channel/new_message", {
        id: channelId,
        message: {
...
QUnit.test("receive new chat messages: out of thrive focus (tab title)", async (assert) => {
    let step = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with thrive out-of-focused
    ...
    // simulate receiving a new message in chat 2 with thrive out-of-focused
    ...
    async () => {
        ...
        pyEnv["discuss.channel"].create([
            ...
            },
            {
                channel_member_ids: [
                    Command.create({ partner_id: pyEnv.currentPartnerId }),
                    Command.create({ partner_id: TEST_USER_IDS.thrivebotId }),
                ],
                channel_type: "chat",
            },
        ]);
        ...
        await click(".o-mail-DiscussSidebarChannel", { text: "ThriveBot" });
