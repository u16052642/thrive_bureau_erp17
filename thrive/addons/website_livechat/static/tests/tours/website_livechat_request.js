...
thrive.loader.bus.addEventListener("module-started", (e) => {
    if (e.detail.moduleName === "@im_livechat/embed/common/livechat_service") {
        const { LivechatService, THRIVE_VERSION_KEY } = e.detail.module;
        // Livechat sessions are deleted if they come from a different thrive version than
        ...
        patch(LivechatService.prototype, {
            async initialize() {
                try {
                    ...
                    localStorage.setItem(THRIVE_VERSION_KEY, serverVersion);
                } catch {
