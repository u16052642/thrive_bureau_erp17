...
from thrive.tools import email_normalize, ReadonlyDict
import logging
...
class GoogleEvent(abc.Set):
    ...
    def get_thrive_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.thrive_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.thrive_id(self.env)).base_event_id
