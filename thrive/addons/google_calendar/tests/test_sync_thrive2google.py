...
class TestSyncThrive2Google(TestSyncGoogle):
    ...
    def test_all_event_updated(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_thrive_id' % self.env.cr.dbname: new_recurrence.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
        }, timeout=3)
    ...
    def test_recurrence_delete_single_events(self):
        ...
        self.assertGoogleEventPatched(event_1.google_id, {
            ...
            'organizer': {'email': 'thrivebot@example.com', 'self': True},
            'attendees': [{'email': 'thrivebot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_thrive_id' % self.env.cr.dbname: event_1.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
            'status': 'cancelled'
        }, timeout=3)
    ...
    def test_create_event_with_sync_config_paused(self):
        ...
        record._sync_thrive2google(self.google_service)
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertEqual(record.name, "Updated Event", "Assert that event name was updated in Thrive Calendar")
        self.assertTrue(record.need_sync, "Sync variable must be true for updating event when sync re-activates")
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        """
        Deletes a synced event with synchronization paused, event must be archived in Thrive and
        ...
        # Assert that synchronization is paused, delete wasn't called and record was archived in Thrive.
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(record.active, "Event must be archived in Thrive after unlinking it")
