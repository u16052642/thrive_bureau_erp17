...
class TestSyncGoogle2Thrive(TestSyncGoogle):
    ...
    def test_recurrence_no_duplicate(self):
        ...
        self.env['calendar.recurrence']._sync_google2thrive(google_events)
    ...
    def test_partner_order(self):
        self.private_partner.email = "internal_user@thrivebureau.com"
        ...
        values = {
            'id': 'oj44nep1ldf8a3ll02uip0c9aa',
            'description': 'Small mini desc',
            'organizer': {'email': 'internal_user@thrivebureau.com'},
            'summary': 'Pricing new update',
            'visibility': 'public',
            'attendees': [{
                'displayName': 'Mitchell Admin',
        ...
        self.env['calendar.event'].with_user(user)._sync_google2thrive(GoogleEvent([values]))
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_recurrence_range_start_date_in_other_dst_period(self):
        ...
        self.env['calendar.recurrence']._sync_google2thrive(google_event)
    ...
    def test_recurrence_edit_specific_event(self):
        ...
        self.env['calendar.recurrence']._sync_google2thrive(recurrent_events)
        ...
        self.env['calendar.event']._sync_google2thrive(specific_event)
        events = self.env["calendar.event"].search(
            [
                ("name", "=", specific_event.summary),
                ("start", ">=", day_start),
    ...
    def test_event_guest_modify_permission(self):
        ...
        This test states that Thrive Calendar:
        ...
        # Sync events from Google to Thrive Bureau ERP and get them after sync.
        self.env['calendar.event']._sync_google2thrive(GoogleEvent([not_editable_event_values, editable_event_values]))
        ...
        # Assert that event is created in Thrive with proper values for guests_readonly variable.
    ...
    def test_attendee_status_is_not_updated_when_syncing_and_time_data_is_not_changed(self):
        ...
        self.env['calendar.recurrence'].with_user(other_user)._sync_google2thrive(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
