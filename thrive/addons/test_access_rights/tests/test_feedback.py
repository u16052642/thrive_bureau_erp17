...
class TestIRRuleFeedback(Feedback):
    ...
    def debug_mode(self):
        thrive.http._request_stack.push(Mock(db=self.env.cr.dbname, env=self.env, debug=True))
        self.addCleanup(thrive.http._request_stack.pop)
        self.env.flush_all()
...
Note: this might be a multi-company issue. Switching company may help - in Thrive, not in real life!
