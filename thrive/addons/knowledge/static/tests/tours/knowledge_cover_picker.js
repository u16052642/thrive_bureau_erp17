...
registry.category("web_tour.tours").add('knowledge_cover_selector_tour', {
    ...
    // Change the search query to find thrive_logo file
    trigger: '.modal-body input.o_we_search',
    extra_trigger: '.modal-body .o_nocontent_help',
    run: 'text thrive_logo',
}, {
    // Choose the thrive_logo cover
    ...
    extra_trigger: '.o_article_active:contains("thrive")',
}, {
    ...
    // Check thrive logo is not shown as the search query does not match its name
    // and remove search query
    trigger: '.modal-body input.o_we_search',
    extra_trigger: '.modal-body .o_nocontent_help',
    run: 'remove_text',
}, {
    // Check that thrive logo is now shown in the cover selector, and make the trash
    // button visible
    trigger: '.modal-body .o_existing_attachment_cell img[title="thrive_logo.png"]',
    run() {},
}, {
    // Click on delete cover button
    trigger: '.modal-body .o_existing_attachment_cell:has(img[title="thrive_logo.png"]) .o_existing_attachment_remove',
    allowInvisible: true,
}, {
