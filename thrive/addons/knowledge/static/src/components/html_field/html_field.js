...
const HtmlFieldPatch = {
    ...
    get valueContainerElement() {
        ...
        } else if (this.wysiwyg?.thriveEditor) {
            return this.wysiwyg.thriveEditor.editable;
        }
        return null;
    },
     ...
     * ThriveEditor has no hook to tell if a node will be removed or not.
    ...
    async _mountBehaviors(behaviorsData, target) {
        ...
        for (const behaviorData of behaviorsData) {
            ...
            // Extract the configuration from the Thrive main Owl App to use it
    ...
    _prepareBehaviorAnchor(behaviorData, renderingContainerElement) {
        ...
        if (this.props.readonly) {
            ...
            behaviorData.insert = (anchor) => {
                if (this.wysiwyg.thriveEditor.editable.contains(behaviorData.anchor)) {
                    // Ignore the insertion if the mounted element
                    // cannot be moved in the DOM.
                    this.wysiwyg.thriveEditor.observerUnactive('mount_knowledge_behaviors');
                    ...
                    this.wysiwyg.thriveEditor.observerActive('mount_knowledge_behaviors');
                    return [anchor];
                }
            };
        }
        ...
        if (!this.props.readonly) {
            ...
            props.onReadyToInsertInEditor = () => {
                ...
                const insertedNodes = behaviorData.insert ?
                    behaviorData.insert(anchor) :
                    this.wysiwyg.thriveEditor.execCommand('insert', anchor);
                if (!insertedNodes) {
                    return cancelInsertion();
                }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await thrive.loader.modules.get('@knowledge/js/wysiwyg');
        if (!wysiwygModule) {
    ...
    _addMountBehaviorsListeners() {
        if (this.wysiwyg?.thriveEditor?.editable) {
            this.wysiwyg.thriveEditor.editable.addEventListener('mount_knowledge_behaviors', this.boundMountBehaviors);
        }
    },
    ...
    _removeMountBehaviorsListeners() {
        if (this.wysiwyg?.thriveEditor?.editable) {
            this.wysiwyg.thriveEditor.editable.removeEventListener('mount_knowledge_behaviors', this.boundMountBehaviors);
        }
    },
};
