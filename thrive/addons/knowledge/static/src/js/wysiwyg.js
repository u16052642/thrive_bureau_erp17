...
patch(Wysiwyg.prototype, {
    ...
    appendBehaviorBlueprint(behaviorBlueprint) {
        const restoreSelection = () => {
            ...
            return setCursorEnd(this.thriveEditor.editable, false);
        }
        const insert = (anchor) => {
            const fragment = this.thriveEditor.document.createDocumentFragment();
            // Add a P after the Behavior to be able to continue typing
            // after it
            const p = this.thriveEditor.document.createElement('p');
            p.append(this.thriveEditor.document.createElement('br'));
            fragment.append(anchor, p);
            const insertedNodes = this.thriveEditor.execCommand('insert', fragment);
            if (insertedNodes) {
    ...
    _insertItemCalendar: function () {
        const { ItemCalendarPropsDialog } = thrive.loader.modules.get("@knowledge/components/item_calendar_props_dialog/item_calendar_props_dialog");
        const restoreSelection = preserveCursor(this.thriveEditor.document);
