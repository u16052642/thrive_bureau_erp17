...
from thrive.addons.sale_subscription.models.sale_order import SaleOrder
...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_subscription_change_partner(self):
        ...
        with patch('thrive.addons.sale_subscription.models.sale_order.SaleOrder.action_confirm', _action_confirm_mock):
    ...
    def test_sale_subscription_post_invoice(self):
        ...
        with patch('thrive.addons.sale_subscription.models.sale_order_line.SaleOrderLine._reset_subscription_quantity_post_invoice', patched_reset), freeze_time("2021-01-01"):
            sub = self.subscription
            sub.action_confirm()
            self.env['sale.order']._cron_recurring_create_invoice()
    ...
    def test_close_reason_automatic_renewal_failed(self):
        ...
        with freeze_time(start_date + relativedelta(days=sub.plan_id.auto_close_limit)):
            with patch('thrive.addons.sale_subscription.models.sale_order.SaleOrder._do_payment', wraps=self._mock_subscription_do_payment_rejected):
                sub._create_recurring_invoice()
