...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_priorities(self):
        ...
        simple Thrive DB so we have to spoof the FROM otherwise we cannot send
        any email. """
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the thrive-bin arguments. This config
        ...
        thrive-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']
        ...
        # for from_filter in ICP, overwrite the one from thrive-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the thrive-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the thrive-bin argument
